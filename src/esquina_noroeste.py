...
if __name__ == '__main__':
    ...
    esquina_noro = pd.read_csv(os.path.join(
        path, 'data', 'data.csv'))  # lee el archivo
    ...
    while True:
        ...
        if dem != 0 and ofer != 0:
            ...
            if dem > ofer:
                ...
                fun_obj += (esquina_noro.iloc[0, 0] * ofer)
                # Se le quita la oferta a la oferta
                esquina_noro.iloc[0,
                                  ind_oferta] = esquina_noro.iloc[0, ind_oferta] - ofer
                # Se le quita la oferta a la demanda
                esquina_noro.iloc[ind_demanda,
                                  0] = esquina_noro.iloc[ind_demanda, 0] - ofer
                ...
                esquina_noro.iloc[0,
                                  ind_oferta] = esquina_noro.iloc[0, ind_oferta] - dem
                # Se le quita la demanda a la demanda
                esquina_noro.iloc[ind_demanda,
                                  0] = esquina_noro.iloc[ind_demanda, 0] - dem
                esquina_noro.drop(
                    esquina_noro.columns[0], inplace=True, axis=1)
                y += 1
        else:
